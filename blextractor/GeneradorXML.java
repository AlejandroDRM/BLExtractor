...
public class GeneradorXML {
    ...
    public void generarXML(String viajeID, ArrayList<String> bls, String packagesQty,
            String containerQty, String netWeight, String grossWeightTotal) {
        DataBase db = new DataBase();
        db.conectar();
        Viaje viaje = db.obtenerViajeCompletoSeleccionBLS(viajeID, bls);
        String numeroContenedores = this.calcularNumeroContenedores(viaje);
        FileWriter fw;
        try {
...
//            fw.write("\n\t</MDS2>");
            ...
            fw.write("\n\t\t<CAR_NET_TON>" + netWeight + "</CAR_NET_TON>");
            fw.write("\n\t\t<CAR_GROS_TON>" + netWeight + "</CAR_GROS_TON>");
            fw.write("\n\t\t<CAR_BL_NBER>" + viaje.getViajeBLS().size() + "</CAR_BL_NBER>");
            fw.write("\n\t\t<CAR_PAC_NBER>" + packagesQty + "</CAR_PAC_NBER>");
            fw.write("\n\t\t<CAR_GROS_MASS>" + grossWeightTotal + "</CAR_GROS_MASS>");
            fw.write("\n\t\t<CAR_CTNR_NBR>" + containerQty + "</CAR_CTNR_NBR>");
    ...
    public String calcularNumeroContenedores(Viaje viaje) {
        String res = "";
        int suma = 0;
        for (ArrayList<Contenedor> grupoContenedor : viaje.getViajeContenedores()) {
